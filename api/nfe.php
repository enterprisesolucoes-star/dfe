            ...
            if ($resultado->sucesso) {
                ...
                if (function_exists('registrarAuditoria')) {
                    registrarAuditoria(
                        ...
                        "NF-e emitida — Nº " . ($venda['numero'] ?? '?') . " | Valor R$ " . number_format($venda['valorTotal'] ?? 0, 2, ',', '.'),
                        null,
                        ['status' => $statusSalvar, 'numero' => $venda['numero'] ?? null, 'chave' => $chaveAcesso, 'protocolo' => $resultado->protocolo ?? '', 'valor_total' => $venda['valorTotal'] ?? 0]
                    );
                }
