        ...
        foreach ($pagamentos as $pag) {
            ...
            if (in_array($forma, $formasCaixa) && $contaIdEfetivo) {
                ...
                $pdo->prepare("
                    ...
                    VALUES (?, ?, ?, 'C', ?, ?, ?, ?)
                ")->execute([
                    $empresaId, $vendaId, $contaIdEfetivo, $valor,
...
// ── PDF do pedido para link direto ────────────────────────────────────────────
if ($action === 'pedido_pdf') {
    $id = (int)($_GET['id'] ?? 0);
    $stmt = $pdo->prepare("SELECT v.*, vd.nome AS vendedor_nome FROM vendas v LEFT JOIN vendedores vd ON vd.id = v.vendedor_id WHERE v.id = ? AND v.empresa_id = ?");
    $stmt->execute([$id, $empresaId]);
    $venda = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$venda) { http_response_code(404); echo 'Pedido não encontrado'; exit; }
    ...
    $pags = $pdo->prepare("SELECT * FROM vendas_pagamentos WHERE venda_id = ?");
    $pags->execute([$id]);
    $pagsList = $pags->fetchAll(PDO::FETCH_ASSOC);
    ...
    $numero = str_pad($venda['numero'], 6, '0', STR_PAD_LEFT);
    $formaLabel = ['01'=>'Dinheiro','02'=>'Cheque','03'=>'Cartão Crédito','04'=>'Cartão Débito','05'=>'Crédito Loja','10'=>'Vale Alimentação','11'=>'Vale Refeição','15'=>'Boleto','17'=>'PIX','99'=>'Outros'];

    $itensHtml = '';
    foreach ($itensList as $i => $it) {
        $itensHtml .= "<tr><td>".($i+1)."</td><td>".htmlspecialchars($it['descricao']??'Produto')."</td><td>".htmlspecialchars($it['unidade']??'UN')."</td><td style='text-align:right'>".number_format($it['quantidade'],3,',','.')."</td><td style='text-align:right'>R$ ".number_format($it['valor_unitario'],2,',','.')."</td><td style='text-align:right'><strong>R$ ".number_format($it['valor_total'],2,',','.')."</strong></td></tr>";
    }
    $pagsHtml = '';
    foreach ($pagsList as $p) {
        $label = $formaLabel[$p['forma_pagamento']] ?? $p['forma_pagamento'];
        $pagsHtml .= "<tr><td>$label</td><td style='text-align:right'>R$ ".number_format($p['valor_pagamento'],2,',','.')."</td></tr>";
    }

    $html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Pedido #'.$numero.'</title>
...
<div class="header"><h1>'.htmlspecialchars($empresa['razao_social']).'</h1>
'.($empresa['cnpj'] ? '<p>CNPJ: '.$empresa['cnpj'].'</p>' : '').'
'.($empresa['telefone'] ? '<p>Tel: '.$empresa['telefone'].'</p>' : '').'</div>
<div class="sem-fiscal">⚠ DOCUMENTO SEM VALOR FISCAL — NÃO É NOTA FISCAL</div>
<div style="margin-bottom:16px">
<div style="font-size:14pt;font-weight:bold;color:#1a56db">PEDIDO Nº '.$numero.'</div>
<p style="font-size:10pt;color:#555">Emitido em: '.date('d/m/Y H:i', strtotime($venda['data_emissao'])).'</p>
'.($venda['natureza_operacao'] ? '<p style="font-size:10pt;color:#555">Natureza: '.htmlspecialchars($venda['natureza_operacao']).'</p>' : '').'
'.($venda['vendedor_nome'] ? '<p style="font-size:10pt;color:#555">Vendedor: '.htmlspecialchars($venda['vendedor_nome']).'</p>' : '').'</div>
'.($venda['cliente_nome'] ? '<div style="border:1px solid #ddd;border-radius:4px;padding:10px;margin-bottom:16px"><strong>Cliente:</strong> '.htmlspecialchars($venda['cliente_nome']).($venda['cliente_documento'] ? ' — '.$venda['cliente_documento'] : '').'</div>' : '').'
...
'.($venda['valor_frete'] > 0 ? '<tr><td>Frete</td><td style="text-align:right">R$ '.number_format($venda['valor_frete'],2,',','.').'</td></tr>' : '').'
<tr class="total-final"><td>TOTAL DO PEDIDO</td><td style="text-align:right">R$ '.number_format($venda['valor_total'],2,',','.').'</td></tr>
</tbody></table>
...
'.($venda['observacao'] ? '<div style="border:1px solid #ddd;border-radius:4px;padding:10px;margin-top:16px"><strong>Obs:</strong> '.htmlspecialchars($venda['observacao']).'</div>' : '').'
