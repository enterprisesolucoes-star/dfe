...
export const FinanceiroView = ({ tipo, emitente, showAlert, showConfirm, cobrancaAtiva = false, initPreset = null }: { tipo: 'R' | 'P', emitente: any, showAlert: any, showConfirm: any, cobrancaAtiva?: boolean, initPreset?: any }) => {
  const [titulos, setTitulos] = useState<any[]>([]);
  const [totPendente, setTotPendente] = useState(0);
  const [totPago, setTotPago] = useState(0);
  const [loading, setLoading] = useState(true);
  const [filtroStatus, setFiltroStatus] = useState<'Pendente' | 'Pago' | ''>(initPreset?.status || (initPreset === 'vencidas' ? 'Pendente' : ''));
  const [di, setDi] = useState(() => {
    if(initPreset?.di) return initPreset.di;
  ...
  const [busca, setBusca] = useState(initPreset?.busca || '');
