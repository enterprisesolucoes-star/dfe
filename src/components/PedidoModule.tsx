...
<script>window.onload=()=>{window.print()}</script></body></html>`;
  ...
  const ic = 'w-full border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none shadow-sm';
  const TABS = [
  ...
  if (modo === 'lista') return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100">Pedidos</h2>
          <p className="text-xs text-amber-600 dark:text-amber-400 font-medium mt-0.5">⚠ Documentos sem valor fiscal</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
          ...
          <p className="text-lg font-bold text-blue-600 dark:text-blue-400">{pedidos.length}</p>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
          <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Total Valor</p>
          <p className="text-lg font-bold text-green-600 dark:text-green-400">R$ {brl(pedidos.reduce((s,p) => s + Number(p.valor_total), 0))}</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="p-4 border-b border-gray-100 dark:border-gray-700 flex flex-wrap gap-3 items-center">
          <div className="relative flex-1 min-w-[200px] max-w-sm">
            ...
            <input value={busca} onChange={e => setBusca(e.target.value)}
              ...
              className="w-full pl-10 pr-4 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500/20" />
          </div>
          ...
          <button onClick={carregarPedidos} className="p-2 text-gray-400 dark:text-gray-500 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg">
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
            <tr>
              <th className="px-5 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Nº</th>
              <th className="px-5 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Data</th>
              <th className="px-5 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Natureza / Cliente</th>
              <th className="px-5 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Vendedor</th>
              <th className="px-5 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase text-right">Valor</th>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {loading ? (
              <tr><td colSpan={6} className="px-5 py-10 text-center"><RefreshCw className="w-5 h-5 animate-spin mx-auto text-gray-300 dark:text-gray-600" /></td></tr>
            ) : pedidosFiltrados.length === 0 ? (
              <tr><td colSpan={6} className="px-5 py-10 text-center text-gray-400 dark:text-gray-500">Nenhum pedido encontrado. Clique em "Novo Pedido" para começar.</td></tr>
            ) : pedidosFiltrados.map(p => (
              <tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                <td className="px-5 py-3 font-bold text-blue-600 dark:text-blue-400">#{String(p.numero).padStart(6,'0')}</td>
                <td className="px-5 py-3 text-gray-500 dark:text-gray-400 text-xs">{new Date(p.data_emissao).toLocaleDateString('pt-BR')}</td>
                <td className="px-5 py-3">
                  <p className="font-medium text-gray-800 dark:text-gray-100">{p.cliente_nome || <span className="text-gray-300 dark:text-gray-600 italic text-xs">Consumidor</span>}</p>
                </td>
                <td className="px-5 py-3 text-gray-500 dark:text-gray-400 text-xs">{p.vendedor_nome || '—'}</td>
                <td className="px-5 py-3 text-right font-semibold text-gray-800 dark:text-gray-100">R$ {brl(Number(p.valor_total))}</td>
                <td className="px-5 py-3 text-center">
                  <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-orange-100 dark:bg-orange-900/40 text-orange-700 dark:text-orange-300">{p.status}</span>
                </td>
                <td className="px-5 py-3">
                  <div className="flex gap-1 justify-center">
                    <button onClick={() => handleImprimir2(p.id)} title="Imprimir" className="p-1.5 text-gray-400 dark:text-gray-500 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg"><Printer className="w-4 h-4" /></button>
                    <button onClick={() => handleEmail(p)} title="Enviar E-mail" className="p-1.5 text-gray-400 dark:text-gray-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg"><Mail className="w-4 h-4" /></button>
                    <button onClick={() => handleWhatsApp(p)} title="WhatsApp" className="p-1.5 text-gray-400 dark:text-gray-500 hover:text-green-600 hover:bg-green-50 dark:hover:bg-green-900/30 rounded-lg"><MessageCircle className="w-4 h-4" /></button>
                    <button onClick={() => handleEditar(p)} title="Editar" className="p-1.5 text-gray-400 dark:text-gray-500 hover:text-orange-600 hover:bg-orange-50 rounded-lg"><Edit className="w-4 h-4" /></button>
                    <button onClick={() => handleExcluir(p)} title="Excluir" className="p-1.5 text-gray-400 dark:text-gray-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </td>
              </tr>
            ))}
  ...
  if (modo === 'sucesso') return (
    <div className="space-y-4">
      <section className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-8 shadow-sm text-center">
        <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
          <CheckCircle className="w-8 h-8 text-green-600 dark:text-green-400" />
        </div>
        <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100 mb-1">Pedido Salvo com Sucesso!</h3>
        <p className="text-gray-500 dark:text-gray-400 mb-1">Pedido Nº <strong className="text-blue-600 dark:text-blue-400">{String(pedidoSalvo?.numero).padStart(6,'0')}</strong></p>
        <p className="text-xs text-gray-400 dark:text-gray-500 mb-6">{pedidoSalvo?.data}</p>
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl px-4 py-3 mb-8 text-xs text-amber-700 dark:text-amber-300 font-medium">
        ...
        <div className="flex gap-3 justify-center flex-wrap">
          ...
          <button onClick={iniciarNovo} className="px-8 py-3 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-xl font-medium hover:bg-gray-200 dark:hover:bg-gray-600 flex items-center gap-2">
            <Plus className="w-4 h-4" /> Novo Pedido
          </button>
          <button onClick={() => setModo('lista')} className="px-8 py-3 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-xl font-medium hover:bg-gray-200 dark:hover:bg-gray-600 flex items-center gap-2">
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm">
            <button onClick={() => setModo('lista')} className="text-blue-600 dark:text-blue-400 hover:underline">← Pedidos</button>
            <span className="text-gray-300 dark:text-gray-600">/</span>
            <span className="font-semibold text-gray-800 dark:text-gray-100">{editandoId ? 'Editando Pedido' : 'Novo Pedido'}</span>
          </div>
          <p className="text-xs text-amber-600 dark:text-amber-400 font-medium mt-0.5">⚠ Documento sem valor fiscal</p>
        </div>
        <button onClick={() => setModo('lista')} className="p-2 text-gray-400 dark:text-gray-500 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl" title="Fechar">
          <X className="w-5 h-5" />
        </button>
      </div>
        ...
        {TABS.map((tab, idx) => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold border transition-all ${
              activeTab === tab.id ? 'bg-white dark:bg-gray-800 text-blue-600 dark:text-blue-400 border-blue-200 dark:border-blue-800 shadow-sm' : 'bg-gray-50 dark:bg-gray-900 text-gray-400 dark:text-gray-500 border-gray-100 dark:border-gray-700 hover:text-gray-600 hover:bg-white'
            }`}>
            <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${activeTab === tab.id ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400'}`}>{idx+1}</span>
        ...
        <div className="space-y-4">
          <section className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
            <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100 mb-4 flex items-center gap-2 border-b border-gray-100 dark:border-gray-700 pb-3">
              <User className="w-5 h-5 text-blue-600 dark:text-blue-400" /> Cliente
            </h3>
                ...
                <div className="absolute z-20 top-full left-0 right-0 mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl max-h-60 overflow-y-auto">
                  {clientesFiltrados.map(c => (
                    <button key={c.id} onClick={() => { setClienteSel(c); setBuscaCliente(c.nome); setDropCliente(false); }}
                      className="w-full text-left px-4 py-3 hover:bg-blue-50 dark:hover:bg-blue-900/30 border-b border-gray-50 dark:border-gray-700 last:border-0">
                      <p className="font-semibold text-gray-700 dark:text-gray-200 text-sm">{c.nome}</p>
                      <p className="text-gray-400 dark:text-gray-500 text-xs">{c.documento}</p>
                    </button>
                  ))}
                </div>
              )}
            ...
            {clienteSel ? (
              <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 border border-blue-100 flex items-center justify-between">
                <div>
                  <p className="font-semibold text-gray-800 dark:text-gray-100">{clienteSel.nome}</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">{clienteSel.documento}</p>
                </div>
                <button onClick={() => { setClienteSel(null); setBuscaCliente(''); }} className="p-1 text-gray-400 dark:text-gray-500 hover:text-red-500 rounded-lg"><X className="w-5 h-5" /></button>
              </div>
            ) : (
              <div className="text-center py-8 border-2 border-dashed border-gray-100 dark:border-gray-700 rounded-xl">
                ...
                <p className="text-sm text-gray-400 dark:text-gray-500">Nenhum cliente selecionado (opcional)</p>
              </div>
            )}
          </section>
          <section className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
            <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100 mb-4 flex items-center gap-2 border-b border-gray-100 dark:border-gray-700 pb-3">
              <UserCheck className="w-5 h-5 text-blue-600 dark:text-blue-400" /> Vendedor e Operação
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Observações</label>
        ...
        <div className="space-y-4">
          <section className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
            <div className="flex items-center gap-4 mb-6">
              <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100 flex items-center gap-2 shrink-0"><Package className="w-5 h-5 text-blue-600 dark:text-blue-400" /> Itens do Pedido</h3>
                  ...
                  <div className="absolute z-20 top-full left-0 right-0 mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl max-h-60 overflow-y-auto">
                    {produtosFiltrados.map(p => (
                      <button key={p.id} onClick={() => adicionarProduto(p)}
                        className="w-full text-left px-4 py-3 hover:bg-blue-50 dark:hover:bg-blue-900/30 border-b border-gray-50 dark:border-gray-700 last:border-0 flex justify-between items-center">
                        <div>
                          <p className="font-semibold text-gray-700 dark:text-gray-200 text-sm">{p.descricao}</p>
                          <p className="text-gray-400 dark:text-gray-500 text-[11px]">{p.codigoInterno} · Estoque: <span className={p.estoque && p.estoque > 0 ? 'text-green-600 dark:text-green-400' : 'text-red-500 dark:text-red-400'}>{p.estoque ?? 0}</span></p>
                        </div>
                        <span className="bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 text-xs font-bold px-2 py-1 rounded-lg">R$ {brl(p.valorUnitario)}</span>
                      </button>
                    ))}
                  </div>
                )}
            ...
            {itens.length > 0 ? (
              <div className="overflow-x-auto -mx-6">
                <table className="w-full border-collapse">
                  <thead><tr className="bg-gray-50 dark:bg-gray-900 text-gray-500 dark:text-gray-400 text-xs font-semibold border-y border-gray-100 dark:border-gray-700">
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                    {itens.map((it, idx) => (
                      <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                        <td className="px-6 py-3"><p className="font-semibold text-gray-700 dark:text-gray-200 text-sm">{it.descricao}</p></td>
                        <td className="px-4 py-3 text-center text-xs text-gray-500 dark:text-gray-400">{it.unidade}</td>
                        <td className="px-4 py-3"><input type="number" min="0.001" step="0.001" value={it.quantidade} onChange={e => atualizarItem(idx,'quantidade',parseFloat(e.target.value)||0)} className="w-full border border-gray-200 dark:border-gray-700 rounded-lg px-2 py-1.5 text-center text-sm outline-none focus:ring-1 focus:ring-blue-500" /></td>
                        <td className="px-4 py-3"><input type="number" min="0" step="0.01" value={it.valorUnitario} onChange={e => atualizarItem(idx,'valorUnitario',parseFloat(e.target.value)||0)} className="w-full border border-gray-200 dark:border-gray-700 rounded-lg px-2 py-1.5 text-center text-sm outline-none focus:ring-1 focus:ring-blue-500 text-blue-600 dark:text-blue-400 font-medium" /></td>
                        <td className="px-4 py-3"><input type="number" min="0" step="0.01" value={it.desconto} onChange={e => atualizarItem(idx,'desconto',parseFloat(e.target.value)||0)} className="w-full border border-gray-200 dark:border-gray-700 rounded-lg px-2 py-1.5 text-center text-sm outline-none focus:ring-1 focus:ring-blue-500 text-red-500 dark:text-red-400" /></td>
                        <td className="px-4 py-3 text-center font-bold text-gray-800 dark:text-gray-100 text-sm">R$ {brl(it.valorTotal)}</td>
                        <td className="px-6 py-3 text-right"><button onClick={() => setItens(prev => prev.filter((_,i) => i !== idx))} className="p-2 text-gray-400 dark:text-gray-500 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg"><Trash2 className="w-4 h-4" /></button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="text-center py-16">
                <div className="bg-gray-50 dark:bg-gray-900 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4"><Package className="w-10 h-10 text-gray-300 dark:text-gray-600" /></div>
                <p className="text-gray-400 dark:text-gray-500 font-medium">Nenhum produto adicionado</p>
              </div>
            )}
          </section>
          {itens.length > 0 && (
            <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-4 flex justify-end gap-8 text-sm">
              <div className="text-right"><p className="text-gray-400 dark:text-gray-500 text-xs">Subtotal</p><p className="font-semibold">R$ {brl(totalProdutos)}</p></div>
              {totalDescontos > 0 && <div className="text-right"><p className="text-gray-400 dark:text-gray-500 text-xs">Descontos</p><p className="font-semibold text-red-500 dark:text-red-400">- R$ {brl(totalDescontos)}</p></div>}
              <div className="text-right"><p className="text-blue-600 dark:text-blue-400 text-xs">Total</p><p className="font-bold text-blue-600 dark:text-blue-400 text-lg">R$ {brl(totalProdutos - totalDescontos)}</p></div>
            </div>
          )}
      ...
      {activeTab === 'transporte' && (
        <div className="space-y-4">
          <section className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
            <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100 mb-6 flex items-center gap-2 border-b border-gray-100 dark:border-gray-700 pb-3"><Truck className="w-5 h-5 text-blue-600 dark:text-blue-400" /> Frete e Despesas Adicionais</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div><label className="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Frete (R$)</label><input type="number" min="0" step="0.01" value={valorFrete} onChange={e => setValorFrete(parseFloat(e.target.value)||0)} className={ic+' text-right'} /></div>
              <div><label className="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Seguro (R$)</label><input type="number" min="0" step="0.01" value={valorSeguro} onChange={e => setValorSeguro(parseFloat(e.target.value)||0)} className={ic+' text-right'} /></div>
              <div><label className="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Outras Despesas (R$)</label><input type="number" min="0" step="0.01" value={valorOutras} onChange={e => setValorOutras(parseFloat(e.target.value)||0)} className={ic+' text-right'} /></div>
            </div>
            {(valorFrete > 0 || valorSeguro > 0 || valorOutras > 0) && (
              <div className="mt-6 bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 flex justify-between items-center">
                <p className="text-sm text-blue-700 dark:text-blue-300 font-medium">Total com acréscimos</p>
                <p className="text-lg font-bold text-blue-700 dark:text-blue-300">R$ {brl(totalPedido)}</p>
              </div>
            )}
          </section>
        ...
        <div className="space-y-4">
          <section className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100 flex items-center gap-2"><CreditCard className="w-5 h-5 text-blue-600 dark:text-blue-400" /> Formas de Pagamento</h3>
              <button onClick={adicionarPagamento} className="bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 px-4 py-2 rounded-xl text-xs font-semibold hover:bg-blue-100 flex items-center gap-2"><Plus className="w-4 h-4" /> Adicionar</button>
            </div>
                ...
                <div key={idx} className="space-y-2">
                  <div className="flex gap-4 items-center bg-gray-50 dark:bg-gray-900 p-4 rounded-xl border border-gray-100 dark:border-gray-700">
                    <div className="flex-1">
                      ...
                      <select value={pag.formaPagamento} onChange={e => atualizarPagamento(idx,'formaPagamento',e.target.value)} className="w-full border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-800 outline-none focus:ring-2 focus:ring-blue-500/20">
                    ...
                    <div className="w-48">
                      ...
                      <input type="number" min="0" step="0.01" value={pag.valorPagamento} onChange={e => atualizarPagamento(idx,'valorPagamento',parseFloat(e.target.value)||0)} className="w-full border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm text-right font-semibold outline-none focus:ring-2 focus:ring-blue-500/20" />
                    </div>
                    {['03','04'].includes(pag.formaPagamento) && (<>
                      ...
                      <div className="w-28"><label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Autorização</label>
                        <input value={pag.autorizacao||''} onChange={e => atualizarPagamento(idx,'autorizacao',e.target.value)} className="w-full border border-gray-200 dark:border-gray-700 rounded-lg px-2 py-2 text-xs outline-none focus:ring-1 focus:ring-blue-500" placeholder="Código" />
                      </div>
                    </>)}
                    {['05','02','15'].includes(pag.formaPagamento) && (
                      <button onClick={() => setParcelamentoIdx(idx)} title="Configurar Parcelas"
                        className={`mt-5 p-2 rounded-xl transition-all ${pag.vencimentos?.length ? 'bg-blue-600 text-white' : 'text-gray-400 dark:text-gray-500 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30 border border-gray-200 dark:border-gray-700'}`}>
                        <CalendarDays className="w-4 h-4" />
                      </button>
                    )}
                    <button onClick={() => removerPagamento(idx)} className="mt-5 p-2 text-gray-400 dark:text-gray-500 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg"><Trash2 className="w-4 h-4" /></button>
                  </div>
                  ...
                  <div className="ml-4">
                    {FORMAS_CAIXA.includes(pag.formaPagamento)
                      ? <span className="text-[10px] bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300 border border-green-100 rounded-full px-2 py-0.5 font-medium">✓ Lança no caixa</span>
                      : <span className="text-[10px] bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-300 border border-orange-100 rounded-full px-2 py-0.5 font-medium">→ Contas a Receber</span>}
                  </div>
                </div>
              ))}
              {pagamentos.length === 0 && <p className="text-center py-6 text-gray-400 dark:text-gray-500 text-sm italic">Nenhuma forma de pagamento adicionada</p>}
            </div>
            <div className="bg-blue-50 dark:bg-blue-900/20 rounded-2xl p-5 border border-blue-100">
              <div className="flex justify-between items-center mb-2">
                <p className="text-sm font-semibold text-blue-700 dark:text-blue-300">Total do Pedido</p>
                <p className="text-xl font-bold text-blue-800">R$ {brl(totalPedido)}</p>
              </div>
              <div className="flex justify-between items-center mb-2">
                <p className="text-sm text-blue-600 dark:text-blue-400">Total Informado</p>
                <p className="text-lg font-bold text-blue-700 dark:text-blue-300">R$ {brl(totalPago)}</p>
              </div>
              {troco > 0 && <div className="flex justify-between items-center mt-2 pt-2 border-t border-blue-200 dark:border-blue-800">
          ...
          <div className="flex justify-end">
            <button onClick={editandoId ? handleFinalizarEdicao : handleFinalizar} disabled={salvando || !!validar()}
              className={`px-8 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 ${salvando || validar() ? 'bg-gray-200 dark:bg-gray-700 text-gray-400 dark:text-gray-500' : editandoId ? 'bg-orange-500 text-white hover:bg-orange-600 shadow-md' : 'bg-green-600 text-white hover:bg-green-700 shadow-md'}`}>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[400] p-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100">Alteração Financeira Detectada</h3>
                <p className="text-xs text-gray-500 dark:text-gray-400">O valor do pedido foi alterado</p>
              </div>
            </div>
            <p className="text-sm text-gray-600 dark:text-gray-300 mb-6">Existem lançamentos financeiros pendentes vinculados a este pedido. O que deseja fazer?</p>
            <div className="space-y-3">
              ...
              <button onClick={() => salvarEdicao(modalFinanceiro.payload, 'manter')}
                className="w-full px-4 py-3 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800 rounded-xl text-sm font-bold hover:bg-blue-100 text-left">
              ...
              <button onClick={() => setModalFinanceiro(null)}
                className="w-full px-4 py-3 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-xl text-sm font-medium hover:bg-gray-200 dark:hover:bg-gray-600">
                Cancelar
              </button>
            </div>
          </div>
...
const PedidoParcelamentoModal = ({ total, inicial, onConfirm, onCancel }: {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[300] p-4 backdrop-blur-sm">
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white dark:bg-gray-800 rounded-3xl w-full max-w-xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
        <div className="p-6 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between bg-gray-50 dark:bg-gray-900">
          <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100">Configurar Parcelamento</h3>
        ...
        <div className="p-6 space-y-4 flex-1 overflow-auto">
          <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-2xl flex items-center gap-4">
            <div className="flex-1">
              <label className="block text-[10px] font-bold text-blue-600 dark:text-blue-400 uppercase mb-1">Nº de Parcelas</label>
              <input type="number" min={1} value={numParcelas} onChange={e => setNumParcelas(Math.max(1, Number(e.target.value)))} className="w-full bg-white dark:bg-gray-800 rounded-xl px-4 py-2 font-bold text-blue-800 outline-none focus:ring-2 focus:ring-blue-400 border-0" />
            </div>
          ...
          <div className="border border-gray-100 dark:border-gray-700 rounded-2xl overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 dark:bg-gray-900"><tr>
                <th className="px-4 py-2 text-[10px] font-bold text-gray-400 dark:text-gray-500 text-left">#</th>
                <th className="px-4 py-2 text-[10px] font-bold text-gray-400 dark:text-gray-500 text-left">Vencimento</th>
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-gray-700">
                {parcelas.map((p, idx) => (
                  <tr key={p.numero}>
                    ...
                    <td className="px-4 py-3"><input type="date" value={p.vencimento} onChange={e => { const n=[...parcelas]; n[idx].vencimento=e.target.value; setParcelas(n); }} className="bg-transparent font-medium text-gray-700 dark:text-gray-200 outline-none focus:text-blue-600" /></td>
                    <td className="px-4 py-3 text-right"><input type="text" inputMode="decimal" value={p.valor.toLocaleString('pt-BR',{minimumFractionDigits:2})} onChange={e => { const n=[...parcelas]; n[idx].valor=Number(e.target.value.replace(/\D/g,''))/100; setParcelas(n); }} className="w-28 text-right bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-lg px-2 py-1 font-bold text-gray-700 dark:text-gray-200 outline-none focus:ring-1 focus:ring-blue-500" /></td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          {invalido && <div className="bg-red-50 dark:bg-red-900/20 p-3 rounded-xl flex items-center gap-2 text-red-600 dark:text-red-400 text-xs font-bold"><AlertCircle className="w-4 h-4" />Soma das parcelas (R$ {brl(totalParcelas)}) não confere.</div>}
        </div>
        <div className="p-6 bg-gray-50 dark:bg-gray-900 border-t border-gray-100 dark:border-gray-700 flex gap-4">
          <button onClick={onCancel} className="flex-1 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 font-semibold rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 text-sm">Cancelar</button>
          <button disabled={invalido} onClick={() => onConfirm(parcelas)} className={`flex-1 py-3 rounded-xl font-bold text-sm ${invalido?'bg-gray-200 dark:bg-gray-700 text-gray-400 dark:text-gray-500 cursor-not-allowed':'bg-blue-600 text-white hover:bg-blue-700 shadow-md'}`}>Confirmar</button>
        </div>
      </motion.div>
    </div>
  );
