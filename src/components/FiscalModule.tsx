...
export const VendasTab = ({ vendas, onCancelar, onSincronizar, onRetryTef, onExcluir, onEmailDoc, onDevolucao, emitente }: {
    vendas: Nfce[],
                                ...
                                <tr key={v.id ?? i} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-all">
                                    ...
                                    <td className="px-4 py-3 text-right">
                                        <div className="flex items-center justify-end gap-1">
                                            ...
                                            {v.status === 'Autorizada' && !!(emitente as any)?.integracaowhatsapp && <button onClick={() => setWaTarget({ id: v.id, action: 'danfe', filename: `nfce_${v.numero}.pdf`, caption: `NFCe N\xba ${String(v.numero).padStart(6,'0')}`, phone: ((v as any).cliente_telefone || '').replace(/\D/g,'') })} className="p-1.5 text-gray-400 dark:text-gray-500 hover:text-green-600 dark:hover:text-green-400" title="Enviar WhatsApp"><MessageCircle className="w-3.5 h-3.5" /></button>}
                                            {v.status === 'Autorizada' && onEmailDoc && <button onClick={() => onEmailDoc(v.id, 65)} className="p-1.5 hover:bg-purple-50 rounded-lg text-purple-600 transition-colors" title="Email"><Mail className="w-3.5 h-3.5" /></button>}
                                            ...
                                            {(v.status === 'Autorizada' || v.status === 'Contingencia') && <button onClick={() => window.open(`./api.php?action=danfe&id=${v.id}`, '_blank')} className="p-1.5 text-gray-400 dark:text-gray-500 hover:text-blue-600 dark:hover:text-blue-400" title="DANFE"><FileText className="w-3.5 h-3.5" /></button>}
                                            {(v.status === 'Autorizada' || v.status === 'Contingencia') && <button onClick={() => window.open(`./api.php?action=danfe&id=${v.id}`, '_blank')} className="p-1.5 text-gray-400 dark:text-gray-500 hover:text-blue-600 dark:hover:text-blue-400" title="DANFE"><FileText className="w-3.5 h-3.5" /></button>}
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2 bg-white dark:bg-gray-800 p-4 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700">
                ...
                <button onClick={() => showPrompt && showPrompt('Enviar XML ao Contador', 'Informe o e-mail do contador:', async (email: string) => { if (!email) return; setEmailSending && setEmailSending(true); const r = await fetch(`./api.php?action=nfe_enviar_xml_contador&data_inicio=${di}&data_fim=${df}&email=${encodeURIComponent(email)}`, {method:'GET'}); const d = await r.json(); setEmailSending && setEmailSending(false); if (!d.success) showAlert && showAlert('Erro', d.message || 'Não foi possível enviar'); }, emitente?.emailContador || '')} className="px-3 py-1.5 bg-blue-600 text-white text-xs font-bold rounded-xl hover:bg-blue-700 transition-all flex items-center gap-1"><Send className="w-3 h-3" /> Enviar Contador</button>
                            ...
                            <tr key={n.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-all">
                                ...
                                <td className="px-6 py-4 text-right">
                                    <div className="flex items-center justify-end gap-1">
                                        ...
                                        {n.status === 'Autorizada' && !!(emitente as any)?.integracaowhatsapp && <button onClick={() => setWaTarget({ id: n.id, action: 'nfe_danfe', filename: `nfe_${n.numero}.pdf`, caption: `NFe N\xba ${String(n.numero).padStart(6,'0')}`, phone: (n.cliente_telefone || '').replace(/\D/g,'') })} className="p-1.5 text-gray-400 dark:text-gray-500 hover:text-green-600 dark:hover:text-green-400" title="WhatsApp"><MessageCircle className="w-3.5 h-3.5" /></button>}
                                        {n.status === 'Autorizada' && onEmailDoc && <button onClick={() => onEmailDoc(n.id, 'nfe')} className="p-1.5 hover:bg-purple-50 rounded-lg text-purple-600 transition-colors" title="Email"><Mail className="w-3.5 h-3.5" /></button>}
                            ...
                            <tr key={n.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-all">
                                ...
                                <td className="px-6 py-4 text-right">
                                    <div className="flex items-center justify-end gap-1">
                                        ...
                                        {n.status === 'Autorizada' && onEmailDoc && <button onClick={() => onEmailDoc(n.id, 65)} className="p-1.5 hover:bg-purple-50 rounded-lg text-purple-600 transition-colors" title="Email"><Mail className="w-3.5 h-3.5" /></button>}
