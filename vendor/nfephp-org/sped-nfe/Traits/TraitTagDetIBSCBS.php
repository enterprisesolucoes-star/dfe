    ...
    public function tagIBSCBS(stdClass $std): DOMElement
    {
        ...
        if (!is_null($std->vBC) && is_numeric($std->vBC)) {
            ...
            $this->dom->addChild(
                $gIBSUF,
                "ind_gRed",
                isset($std->gIBSUF_ind_gRed) ? $std->gIBSUF_ind_gRed : 0,
                true,
                "$identificador Indicador de Redução de Alíquota (ind_gRed)"
            );
            ...
            $this->dom->addChild(
                $gIBSMun,
                "ind_gRed",
                isset($std->gIBSMun_ind_gRed) ? $std->gIBSMun_ind_gRed : 0,
                true,
                "$identificador Indicador de Redução de Alíquota (ind_gRed)"
            );
            ...
            $this->dom->addChild(
                $gCBS,
                "ind_gRed",
                isset($std->gCBS_ind_gRed) ? $std->gCBS_ind_gRed : 0,
                true,
                "$identificador Indicador de Redução de Alíquota (ind_gRed)"
            );
